...
class TimerControllerWidget extends StatefulWidget {
  final Crop crop;
  final int index;
  const TimerControllerWidget({Key key, this.crop, this.index})
      : super(key: key);
...
class _TimerControllerWidgetState extends State<TimerControllerWidget> {
        ...
        Expanded(
          ...
          children: [
            IconButton(
                ...
                onPressed:
                    timerService.isRunning(index) ? null : () => start(index)),
            IconButton(
                icon: Icon(Icons.stop, size: 32),
                onPressed:
                    !timerService.isRunning(index) ? null : () => stop(index)),
            IconButton(
  ...
  void stop(int index) {
    ...
    timerSubject.add({'index': index, 'timer': 0, 'stop': true});

    timerSubject.add({'index': index, 'timer': 0, 'stop': false});
